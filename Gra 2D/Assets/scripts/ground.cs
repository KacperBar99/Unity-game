...
public class ground : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        if (collision.tag == "Enemy")
        {
            ...
            {
                collision.GetComponent<Rigidbody2D>().AddForce(new Vector3(-force, 0f));
            }
            else
            {
        ...
        {
            if (collision.GetComponent<power_rock>().type == 1)
        ...
        if (collision.tag == "Door")
        {
            ...
            {
                if (collision.GetComponent<Doors>().cover_type == 1)
                {
